...
| 碩論 v4 docx — §1.1 why-spine 重寫 | 5 段式 why（商業/技術/文獻/缺口/定位），把整篇敘事主軸定錨；含 200 字口試開場白（已封存）|
| 碩論 v5 docx — 引用順序修正 ⭐ | 35 篇引用按出現次序重新編號（過程中發現腳本誤把 LaTeX 區間 `α ∈ [0,1]` 也當引用，已 4 處手動修補；最終 1..35 連續無缺）|
| Downloads 整理 | v2/v3/v4/原稿/ILT2026 偽造版 5 檔移入 `~/Downloads/.archive/`；可見區只剩兩份正典（碩論 v5 + ILT2026 v2 據實版）|

### 當前正典版本（5/9 收工時）

| 用途 | 檔案 |
|------|------|
| 碩論最新版（口試用） | `~/Downloads/坤大碩論_v5_引用順序修正_陳銓富.docx`（572 KB）|
| ILT2026 投稿（4 頁） | `~/Downloads/ILT2026_陳銓富_v2_據實版.docx`（211 KB）|
| Perpetual RAG（另一篇）| `~/paper_KnowledgeBase/論文_IEEE_Overleaf.tex` + 同步 .md/.docx |
| Benchmark v1.0.1 release | https://github.com/quanfu2026/perpetual-rag/releases/tag/v1.0.1 |

---

## ✅ 第五十八次（2026-05-09）— ILT2026 據實版 + 碩論 50 頁擴寫

### 🎯 本次重大里程碑：兩篇論文同步產出

5/6 完成 .tex / .md / .docx 同步後，5/8 用戶送來 ILT2026 投稿稿（4 頁）發現**全部數據是初稿暫填的偽造數據**（95.0%、0.898、5%）。本次完成：(1) 拒絕「修數字」式的學術不端，提出「路線 1：用真實 benchmark 數字重寫」；(2) 寫出 ILT2026 v2 據實版；(3) 發現用戶其實還有崑大碩論版本，要從 6 頁擴寫到 50 頁；(4) 完成七章碩論 + 前頁 + 目錄 + 參考 + 附錄之完整 docx。

### 完成事項（兩篇文章）

#### 一、ILT2026 conference paper 救援（4 頁）

| 任務 | 結果 |
|------|------|
| **發現偽造數據** | 原稿 Hit@5=95%、MRR=0.898、幻覺率 5% 全為初稿暫填 |
| **拒絕學術不端** | 不協助「把假數字修得更像真的」，提出用真實 C-MTEB benchmark 重寫 |
| **修引用錯誤 4 處** | Robertson [9] BM25 vs TF-IDF 偷換概念 / Khattab ColBERT / Devlin BERT / Liu Lost-in-Middle 全為亂引 |
| **修「顯著」未檢定問題** | 0.9pp 差距宣稱顯著無 McNemar，補上正確檢定數字 |
| **刪過度宣稱** | 「智慧農業」應用未做實驗即不可宣稱，刪除 |
| **補 limitation** | 從 0 項補到 4 項 |
| **題目情境 A 鎖死後 pivot** | 標題保留原版「基於雙層檢索機制」，內容 reframe 為「設計並實證雙層檢索機制，發現雙層在此任務無效」 |
| **產出檔案** | `/Users/wenzhongchen/Downloads/ILT2026_陳銓富_v2_據實版.docx`（210 KB，含 2 張圖嵌入）|

#### 二、崑大碩論擴寫（50 頁完整版）

| 章節 | 字數 | 估算頁數 | 重點 |
|------|------|---------|------|
| Ch1 緒論 | 3,472 | ~2.5 | 4 個 RQ 對應第五章 6 條件 |
| Ch2 文獻探討 | 25,419 | ~14 | 9 大節 + §2.9.1 核心研究比較表（10 篇 prior work 三層定位）|
| Ch3 研究方法 | 17,012 | ~10 | 三層分離架構、6 條件公式化、Algorithm 3.1/3.2 pseudocode |
| Ch4 系統設計與實作 | 17,716 | ~10 | **§4.4 完整 Bug case study + 三項方法論啟示** |
| Ch5 實驗結果 | 7,037 | ~4 | 含真實表 5.1–5.6 + 圖 5.1/5.2 |
| Ch6 討論 | 3,797 | ~2.5 | 雙層失效三條件、文獻對話 |
| Ch7 結論 | 2,640 | ~1.5 | 三層次貢獻 + 5 項未來工作 |
| 前頁 + 目錄 + 35 篇參考 + 附錄 A/B | ~12,000 | ~6 | 完整參考文獻、CHANGELOG、4 段程式碼附錄 |
| **小計** | **~89,000** | **~50** | **目標達成** |
| **產出檔案** | | | `/Users/wenzhongchen/Downloads/坤大碩論_v2_50頁完整版_陳銓富.docx`（494 KB）|

### 寫作風格決議

- **中文技術詞後加英文原文**（用戶於 5/9 確立此風格）：例「檢索增強生成（Retrieval-Augmented Generation, RAG）」、「規格代碼召回偏移（specification drift）」
- 全部使用真實 benchmark 數字（93.2% Hit@5），絕不用偽造數字
- 誠實揭露兩項負面 ablation（α\*=0、CE rerank 趨負）
- 緒論 / 各章開頭交代章節結構，符合台灣碩論慣例
- 引文標號 [1]–[35]（碩論完整版整合至 35 篇）

### 待辦（下次開工優先序）

1. **🔴 ILT2026 投稿狀態確認** — 用戶 5/8 截止，未明說是否已投出 v2 據實版；若還沒投，要逼自己今天送出
2. **🔴 碩論 docx 頁數實測** — pandoc 估 50 頁但實際 Word 開檔可能有出入；需用戶確認頁數，過長則砍 Ch4 §4.4 詳論或 Ch2 §2.5–2.6
3. **🟠 Self-plagiarism 揭露** — 若 ILT2026 已投出，碩論第一頁需加註：「本論文部分內容已發表於 ILT2026」
4. **🟠 benchmark v1.0.1 release push** — 5/8 已 commit + tag local，未 push GitHub；需用戶授權 push（含 CHANGELOG）
5. **🟡 碩論 docx 圖表正確性** — 圖 5.1/5.2 已嵌入，但 Word 開檔後需確認位置、大小、解析度
6. **🟡 目錄頁碼校對** — pandoc 產出之 ToC 頁碼是手填的，Word 開檔後可右鍵「更新目錄」自動同步
7. **🟢 參考文獻交叉檢查** — 引文編號 [1]–[35] 正文與參考文獻列表是否一致
8. **🟢 致謝段個人化** — 目前致謝為通用版，用戶可能想加個人感謝對象

### 重要備註

- **ILT2026 與碩論共用 benchmark 數據**：兩篇均用同一份 C-MTEB EcomRetrieval 結果（T+ 93.2%）。若 ILT2026 已投出且接受，碩論應加 self-plagiarism 揭露（這是學術倫理慣例，不算 misconduct）
- **碩論 §4.4 揭露 v1.0.0 草稿曾誤用 0.6% 錯誤數字**：這是論文加分項（research integrity），但若指導教授覺得太誠實則可考慮淡化
- **碩論 §2.9.1 核心研究比較表**寫得特別清楚——10 篇 prior work 之繼承 / 採用 / 挑戰呼應三層結構，對口試委員特別友善
- **Ch4 case study 是論文最不一樣的部分**：把實作 bug 寫進論文是技術型 thesis 的少見做法，敢誠實揭露反而是研究嚴謹性的展現
- 兩篇 docx 之備份分別為 `*.before_v57_sync` (5/6 paper repo) 與 `坤大_*.docx` (5/9 master thesis 原版)；需要 rollback 都還可以

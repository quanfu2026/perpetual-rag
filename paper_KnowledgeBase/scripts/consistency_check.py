...
WHITELIST = {
    ...
    "幻覺率": "幻覺率有三種合理脈絡：(1) 純LLM無RAG時 59%（H1 控制組基準）；(2) 時效性問題純LLM 100%（子項對照）；(3) RAG增強後本研究實測 3%；三者測量情境不同，非矛盾",
    "Hit@5": "Hit@5 僅出現於 REF_ 參考文獻檔案（已排除掃描），論文正文不再主張 Hit@5 實測值",
    "alpha參數": "α 值僅出現於 REF_ 參考文獻檔案（已排除掃描），論文正文不再引用 α 值",
    "MRR": "MRR 僅出現於 REF_ 參考文獻檔案與 Principles_Manual 示範句（均已排除掃描），論文正文不再引用 MRR 實測值",
}
